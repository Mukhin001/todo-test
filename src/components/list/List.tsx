...
const List = ({ arrToDo, setArrToDo, keyShowToDO }: Props) => {
    ...
    const handleEditToDO = (e: React.MouseEvent<HTMLDivElement, MouseEvent>): void => {
        const currentElement = e.currentTarget.dataset.id;
    ...
    const handleCloseEditToDo = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>): void => {
        const currentElement = e.currentTarget.dataset.id;
            ...
            <ul>
                { arrToDo.filter((todo) => filterArrToDo(todo, keyShowToDO)).map(toDo => 
                    <li className={`${st.listLi} ${toDo.showEdit ? st.listLiFocus : ''}`} key={toDo.id}>
                        ...
                        <div data-id={toDo.id} className={`${st.listLi__iconEdit} ${!toDo.showEdit ? st.showEdit : st.hiddenEdit}`} onClick={handleEditToDO}>
                        ...
                        <div className={`${st.editToDO} ${toDo.showEdit ? st.showEdit : st.hiddenEdit}`}>
                            <form className={st.editToDO__form} onSubmit={handleSubmit} data-idform={toDo.id}>
                                ...
                                <div>
                                    ...
                                    <button data-id={toDo.id} type='reset' onClick={handleCloseEditToDo}>X</button>
                                </div>
